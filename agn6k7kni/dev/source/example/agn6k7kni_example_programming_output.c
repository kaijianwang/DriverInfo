/***************************************************************************** 
 *  (c)  2014, National Instruments, Corporation.  All Rights Reserved.      * 
 *****************************************************************************/

/*****************************************************************************
 *  agn6k7kni Instrument Driver Example Program                              
 *  Developed with LabWindows/CVI                                   
 *
 *  Purpose:  This sample program initializes the power supply, configures
 *            a output of the APS, and displays a message pop-up dialog.
 *
 *            To run this sample program, do the following:
 *            1) Create a new project in LabWindows/CVI.
 *            2) Add this file to the project.  To add files to the project,
 *               select the Add Files To Project from the Edit menu of the 
 *               Project window.
 *            3) Add one of the following files to the project:
 *               agn6k7kni.fp, agn6k7kni.c, or agn6k7kni.lib.
 *            4) Check that the resource name used in agn6k7kni_InitWithOptions()
 *               is correct. If simulating, change flag to simulate.
 *            5) Run the project.
 *
 *            VCC or Borland Users:
 *            This example uses CVI's MessagePopup() to display the
 *            measured value.  You will need to replace this function with a 
 *            printf() or similar display function.  You will then need to 
 *            remove the #include <userint.h> statement.  You can then follow 
 *            the above directions for CVI users.
 *
 *****************************************************************************/

#include <utility.h>
#include <userint.h>
#include <stdio.h>
#include "agn6k7kni.h"

void BuildErrorString (ViSession agn6k7kni, ViStatus error, ViString errStr);

int main (void)
{
    ViSession   agn6k7kni = VI_NULL;
    ViStatus    error = VI_SUCCESS;
    ViBoolean   overVoltage = VI_FALSE, overCurrent = VI_FALSE, constantVoltage = VI_FALSE, constantCurrent = VI_FALSE;
    ViChar      popUpMsg[1024];

	popUpMsg[0] = '\0';
	/* 
	 If you want to run this sample program and the instrument is not present, set the Simulate flag to 1.
     ( Example for single slots: "Simulate=0,RangeCheck=1,QueryInstrStatus=0,Cache=1,DriverSetup=Model:N7952A",)
	   Example for empty or double slots: "Simulate=1,RangeCheck=1,QueryInstrStatus=0,Cache=1,DriverSetup=Model:N7952A",)			
     */
    checkErr( agn6k7kni_InitWithOptions (
                                            "TCPIP0::A-N7952A-40001::inst0::INSTR",	/* resourceName */
                                            VI_TRUE,			/* IdQuery */
                                            VI_TRUE,			/* resetDevice */
                                            "Simulate=0,RangeCheck=1,QueryInstrStatus=0,Cache=1",
                                            &agn6k7kni)
                                        );
    
    /*- Configure output settings. -------------------------------------------------------------------*/
    
    /* To specify voltage priority mode */
	checkErr( agn6k7kni_ConfigureOutputPriorityMode (agn6k7kni, "1", AGN6K7KNI_VAL_FUNCTION_VOLTAGE));
    /* To set the output voltage to 30 volts */
    checkErr( agn6k7kni_ConfigureVoltageLevel (agn6k7kni, "1", 30.0));
    /* To set the positive current limit */
    checkErr( agn6k7kni_ConfigureCurrentLimit (agn6k7kni, "1", VI_NULL, 12.0));
    /* To set the negative current limit */
    checkErr( agn6k7kni_ConfigureNegativeCurrentLimit (agn6k7kni, "1", -1.0));
    /* To set the fastest slew rate */
    checkErr( agn6k7kni_ConfigureSlewRate (agn6k7kni, "1", AGN6K7KNI_VAL_SLEW_RATE_MAX));
    /* To select the high bandwidth */
    checkErr( agn6k7kni_ConfigureOutputBandwidth (agn6k7kni, "1", AGN6K7KNI_VAL_VOLTAGE_BANDWIDTH_HIGH));
    /* To enable output resistance and select a resistance of 0.5 ohms */
    checkErr( agn6k7kni_ConfigureOutputResistance (agn6k7kni, "1", VI_TRUE, 0.5));
    /* To enable the output */
    checkErr( agn6k7kni_ConfigureOutputEnabled (agn6k7kni, "1", VI_TRUE));
    
    /*- Configure output protection settings. --------------------------------------------------------*/
    
    /* To set the OVP level to 40 volts */
    checkErr( agn6k7kni_ConfigureOVP (agn6k7kni, "1", VI_TRUE, 40.0));
    /* To enable OVP and specify a 10 millisecond delay and start the delay timer by an output settings change */
    checkErr( agn6k7kni_ConfigureOCP (agn6k7kni, "1", VI_TRUE, 0.01, AGN6K7KNI_VAL_OCP_DELAY_START_SCHANGE));
    /* To enable the output watchdog timer and set the output watchdog timer to 120 seconds */
    checkErr( agn6k7kni_ConfigureWatchdogTimer (agn6k7kni, VI_TRUE, 120));
    
    /*- Read output state. ---------------------------------------------------------------------------*/
    
	Delay(2.0);

    checkErr( agn6k7kni_QueryOutputState (agn6k7kni, "1", AGN6K7KNI_VAL_OUTPUT_OVER_VOLTAGE, &overVoltage));
	checkErr( agn6k7kni_QueryOutputState (agn6k7kni, "1", AGN6K7KNI_VAL_OUTPUT_OVER_CURRENT, &overCurrent));
	checkErr( agn6k7kni_QueryOutputState (agn6k7kni, "1", AGN6K7KNI_VAL_OUTPUT_CONSTANT_VOLTAGE, &constantVoltage));
	checkErr( agn6k7kni_QueryOutputState (agn6k7kni, "1", AGN6K7KNI_VAL_OUTPUT_CONSTANT_CURRENT, &constantCurrent));


	checkErr( Ivi_SetAttributeViBoolean (agn6k7kni, "1", IVIDCPWR_ATTR_OUTPUT_ENABLED, 0, VI_FALSE));

    popUpMsg[0] = 0;
	if (overVoltage || overCurrent)
    {
		if (overVoltage)
			sprintf (popUpMsg, "%s Channel 1 over-voltage condition occurred.\n", popUpMsg);
		if (overCurrent)
			sprintf (popUpMsg, "%s Channel 1 over-current condition occurred.\n", popUpMsg);
    }
    else
    {
		sprintf (popUpMsg, "The power supply is generating the power you configured.\n");
		if (constantVoltage)
			sprintf (popUpMsg, "%s Channel 1 is operating in CV mode.\n", popUpMsg);
		if (constantCurrent)
			sprintf (popUpMsg, "%s Channel 1 is operating in CC mode.\n", popUpMsg);
    }

#ifdef _CVI_
	MessagePopup ("Message", popUpMsg);
#else
	printf("Message:\n%s", popUpMsg);
#endif
	

Error:
    if (error != VI_SUCCESS)
        {
            ViChar   errStr[2048];

            BuildErrorString (agn6k7kni, error, errStr);
#ifdef _CVI_
            MessagePopup ("Error!", errStr);
#else
			printf("Message:\n%s", errStr);
#endif
        }

    agn6k7kni_ConfigureOutputEnabled (agn6k7kni, "1", VI_FALSE);

    if (overVoltage || overCurrent)
		agn6k7kni_ResetOutputProtection (agn6k7kni, "1");
        
    if (agn6k7kni)
        agn6k7kni_close (agn6k7kni);
	
	return 0;
}

/*****************************************************************************
 * Function: BuildErrorString   
 * Purpose:  This function builds a string that describes the error that the 
 *           sample program encounters using the error information functions 
 *           that the instrument driver exports.
 *****************************************************************************/
void BuildErrorString (ViSession agn6k7kni, ViStatus error, ViString errStr)
{

    ViChar   tempStr[256], errElabStr[256];
    ViChar   *p = errStr;
    ViStatus primaryErr, secondaryErr;
            
    p += sprintf (p, "Driver Status:  (Hex 0x%x)", error);
                                
        /* Get description of the returned status code */
    agn6k7kni_error_message (agn6k7kni, error, tempStr);
    p += sprintf (p, " %s\n\n", tempStr);
        
        /* Get any additional error information */
    errElabStr[0] = 0;
    agn6k7kni_GetErrorInfo (agn6k7kni, &primaryErr, &secondaryErr, errElabStr);
    
    if (primaryErr == error)
        {
        if (secondaryErr != VI_SUCCESS)
            {
                p += sprintf (p, "Secondary Error:  (Hex 0x%x)", secondaryErr);
                                
                    /* Get description of the returned status code */
                agn6k7kni_error_message (agn6k7kni, secondaryErr, tempStr);
                p += sprintf (p, " %s\n\n", tempStr);
            }
        if (errElabStr[0] != 0)
            sprintf (p, "Elaboration:  %s", errElabStr);
        }
}
