                                    ...
                                    ViBoolean reset, ViSession vi)
{
    ...
    if (!Ivi_Simulating(vi))
    {
		...
		if (IDQuery)                               
		{
			...
			if (strncmp (rdBuffer, VALID_RESPONSE_STRING_START_AFG3021B, strlen(VALID_RESPONSE_STRING_START_AFG3021B)) == 0 )
		    {             
                                                                    ...
                                                                    ViReal64 value)
{
	...
	if( value > (dutyCycle*period/0.625-trailingTime) )
	{
		error = IVI_ERROR_INVALID_VALUE;
		viCheckErr (error);
	}
	
	if( value > (period*(1-dutyCycle)/0.8-trailingTime) )
	{
		error = IVI_ERROR_INVALID_VALUE;
		viCheckErr (error);
	}
                                                                     ...
                                                                     ViReal64 value)
{
	...
	if( value > (dutyCycle*period/0.625-leadingTime) )
	{
		error = IVI_ERROR_INVALID_VALUE;
		viCheckErr (error);		
	}

	if( value > (period*(1-dutyCycle)/0.8-leadingTime) )
	{
		error = IVI_ERROR_INVALID_VALUE;
		viCheckErr (error);	
	}
                                                                  ...
                                                                  ViReal64 value)
{
	...
	if( value > (period - width - 0.8*(leadingTime+trailingTime)) )
	{
		error = IVI_ERROR_INVALID_VALUE;
		viCheckErr (error);
	}
